...
from .features.time import DefaultTimeGenerator, NormalTimeGenerator
...
env_args = {
    ...
    'reco_max_steps': 100,
    'normal_time_generator': False,
    'random_product_view': False
}
...
class AbstractEnv(gym.Env, ABC):
    ...
    def __init__(self):
        ...
        self.static_rng         = RandomState(123)

    def reset_random_seed(self, seed, epoch = 0):
        # Initialize Random State.
        assert (self.config.random_seed is not None)
        self.rng = RandomState(seed + epoch)
    ...
    def init_gym(self, args):
        ...
        if self.config.normal_time_generator:
            self.time_generator = NormalTimeGenerator(self.config)
        else:
            self.time_generator = DefaultTimeGenerator(self.config)

        # Setting random seed for the first time.
        self.reset_random_seed(self.config.random_seed)
    ...
    def generate_organic_sessions(self):
        ...
        while self.state == organic:
            ...
            if self.config.random_product_view:
                self.product_view = self.rng.choice(self.config.num_products)
            else:
    ...
    def step(self, action_id):
        ...
        self.curr_steps += 1
        # Update done flag.
        done = True if self.state == stop else (self.curr_steps >= self.config.reco_max_steps)
    ...
    def step_offline(self, observation, reward, done):
        """Call step function with the policy implemented by a particular Agent."""
