...
from numpy import array, diag, exp, matmul, mod, sqrt, expand_dims
...
class RecoEnv1(AbstractEnv):
    ...
    # def make_feature_dist(self, kind, size = 16):

    #     dist = self.rng.uniform(0, 0.5, size)
    ...
    def reset(self, user_id = 0):
        ...
        # dist = self.rng.uniform(0, 0.5, 16)

        # i = self.rng.randint(0,4) * 4
        # dist[i]   += 0.2
        # dist[i+1] += 0.5
        # dist[i+2] += 0.5
        # dist[i+3] += 0.2
        # dist = expand_dims(dist, axis=1)
        ...
        # print('#'*100)
        # print(self.config.sigma_omega_initial)
        # print(self.omega)
        # sumt = 0
        # for e in self.omega:
        #     sumt += e[0]
